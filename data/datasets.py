# File incldues datasets and Lighning dataset wrappers
...
class WeatherBench(Dataset):
    ...
    def __init__(
        ...
        lat_indices = np.searchsorted(lat_vals, lat2d[:, 0])[
            :, None
        ]  # shape (160, 1), broadcast later
        ...
        lon_indices = np.searchsorted(lon_vals_sorted, lon2d_sorted[0, :])[
            None, :
        ]  # shape (1, 220), broadcast later
    ...
    def get_split(var: str) -> slice:
        """Returns time split of train/val/test data

        Args:
            var (str): Variable

        Raises:
            AssertionError: Variable needs to be in [train,val,test]

        Returns:
            slice: Time slice for era5 and ifs
        """
        if var == "train":
    ...
    def __getitem__(self, idx: int) -> tuple:
        ...
        if self.normalize:
            if self.var == "test":
                input[: self.n_vars] = (
                    input[: self.n_vars] - self.input_mean[:, None]
                ) / self.input_std[:, None]
                ...
                input[: self.n_vars] = (
                    input[: self.n_vars] - self.input_mean
                ) / self.input_std
                target = (target - self.target_mean[0]) / self.target_std[0]
...
class WeatherBenchModule(L.LightningDataModule):
    """Lighning datamodule wrapper for the WeatherBench dataset."""
    ...
    def __init__(
        ...
        Args:
            train_batch_size (int, optional): Batch size. Defaults to 1024.
            num_workers (int, optional): Num workers for dataloader. Defaults to 0.
            data_path (str, optional): Defaults to "/home/groups/ai/datasets/weather_forecasting/".
        ...
        # Get datasets
        self.train_dataset = WeatherBench(
            data_path=self.data_path, var="train", normalize=self.normalize
        )
        self.val_dataset = WeatherBench(
            data_path=self.data_path, var="val", normalize=self.normalize
        )
        self.test_dataset = WeatherBench(
            data_path=self.data_path, var="test", normalize=self.normalize
        )
